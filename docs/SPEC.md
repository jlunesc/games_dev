...
- **M5** (new direction, owner 2026-09-21, details **DELEGATED**), in this order: (1) **boss skills**: movement skills for bosses (jumping, dashing and similar) and, after them, **arena features** (places to stand, and cover that protects from attacks). **M5a** (the owner chose leap and dash as the first two movement skills; teleport later) is **built and awaiting the owner's play test** on the phone: the boss format can describe leaps, dashes in either direction and attacks that only reposition, and the Ashen Hound (section 7) is the sample that uses them (design in `docs/superpowers/specs/2026-09-21-m5a-boss-movement-design.md`, checklist in `docs/phone-testing.md`); (2) **generated bosses**: instead of only hand-named bosses, bosses built at random from a set of skills by a seeded generator, each checked for fairness by simulated players before it is offered; the Ember Duelist stays as the fixed reference boss so statistics have something stable to compare with; (3) the security test. The APK decision is dropped (section 3).
  ...
  - **M5e the boss generator** (built, awaiting the owner's play test; design in `docs/superpowers/specs/2026-09-22-m5e-generator-design.md`, format and mechanism in `docs/bosses.md` section 3b, checklist in `docs/phone-testing.md`). **LOCKED** (owner, 2026-09-22, this and two follow-up rounds): built from a **bank of skill primitives** (the same hit/move/leap effects a hand-written boss file already has), randomized and assembled, rather than a wholly free-form generator, with a **reusable fairness checker** general enough for later steps (arenas, phase 2) without a rewrite; reached through a **"Generated" entry in the Boss row**, and **regenerated fresh from that fight's own seed every time Fight is pressed**, so it needs no saved roster and stays replayable from the seed alone; **v1 scope is no arena and one phase only** for a generated boss; a generated boss **does get a counterable attack**, like the Duelist's slam; if a candidate keeps failing the fairness check, the fight **falls back to a small hand-built boss** (the Trainee) rather than ever starting unfair. **Changed (owner, 2026-09-22):** this fallback was replaced by a banner instead — see the M6a entry below. Everything else was left to Claude (`DELEGATED`): the tuned ranges and the 300 ms / 350 ms readability floor (all in `src/bosses/generate/tuning.ts`, one file to retune it), what the fairness checker checks (an idle player must always lose; a scripted skilled bot must win at least one of two seeds without taking damage), and the three-try-then-fallback mechanism (`resolveBoss` in `src/bosses/resolve.ts`). Measured, not guessed: the fallback is reached about 2% of the time (2 of 100 seeds in a natural sweep), not the "rare, hard to force" the design first guessed before it was measured. **M5, as a whole, is now built**: the study phase (M5b), the arena (M5c), the looks (M5d) and the generator (M5e) are all built.
  - **M6** (owner play test, 2026-09-22): after playing all of M5, the owner asked for seven follow-on changes: (1) a Back row on every menu screen that lacked one, (2) generated arena diversity, (3) new attack primitives (explosions, projectiles), (4) boss archetypes, (5) background/colour diversity, (6) movement diversity, (7) campaign mode. **Superseded the same day** by a phased roadmap after a design conversation about the project's actual point — see the M6 roadmap entry below, which is now the LOCKED order. Item 1 is **built** (on `main`).
  - **The M6 roadmap** (LOCKED, owner, 2026-09-22; full detail in `docs/superpowers/specs/2026-09-22-m6-roadmap-design.md`): the flat M6 order above was replaced after the owner observed that M6a's fairness-checking work had become precision-engineering for its own sake, when the actual point (section 1: transferable skills) only needs a generated boss to be a legitimate, beatable obstacle — not a proven-optimal one. The owner's stated real goal: generate varied attacks/mechanisms, identify what the owner specifically does wrong, and steer future generation toward practicing that — which pulls forward two items that were sitting undesigned since the first spec conversation (this section's own "read stats per attack type and skill" line, and the backlog's "steer generation from play stats" idea). New order: **Phase 0** finish M6a as already designed (arena diversity + the banner-instead-of-fallback fairness redesign, in progress); **Phase 1** five hand-built boss archetypes (zoner, heavy bruiser, trickster, rushdown, counter-bait) using the existing format, no new engineering, played across difficulties for real data; **Phase 2** design the skill-tagging taxonomy and stat-driven weakness analysis from that real data (the long-open item above); **Phase 3** resume new attack primitives and movement diversity, built leaner (cheap fairness backstop only, no repeat of M6a's proof-chasing); **Phase 4** campaign mode, using Phase 1's five bosses as a starting roster; backgrounds slot in anytime, independent of the rest.
  - **M6a generated arenas** (design in `docs/superpowers/specs/2026-09-22-m6a-generated-arenas-design.md`, awaiting the owner's review of that document before it is planned and built). **LOCKED** (owner, 2026-09-22): a generated boss sometimes has an arena and sometimes stays bare, like the Duelist; when it has one, platform and cover heights are spread apart on purpose (the concrete owner feedback that drove this: the Ashen Hound's platform and cover sit only 10 units apart in height, which read as "crowded," not the number of pieces). **DELEGATED**: the exact tuned numbers (bare chance, piece counts, placement zones, the minimum height gap) are in the design doc's `GEN` values; the fairness checker gains a genuine test for the known-but-never-tested risk named in `docs/bosses.md` (an idle player camping on a tall platform), applied generically to any boss with an arena, including a fresh check of the Hound's own shipped arena. **LOCKED** (owner, 2026-09-22): when none of the three tries passes the fairness check, the fight uses the first candidate anyway (never a fallback boss) and shows the player a short banner saying it couldn't be checked as fair, instead of the M5e-era Trainee fallback (`resolveBoss` in `src/bosses/resolve.ts`, banner in `src/ui/study-banner.ts`).
