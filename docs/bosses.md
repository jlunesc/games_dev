...
| An arena piece's `height` | A platform is a place to stand; a cover blocks windows whose `top` is at or below its height. | For anything a player can stand on (a platform, or the top of a cover): a height above every window's `top` makes a safe place to camp, so keep it below the tallest window that should still reach a player standing there (the Hound's platforms are 90 and its cover 100, against tops of 110, 100 and 60). A cover also blocks only the windows whose `top` is at or below its height (the Hound's 100 stops the rush and the shockwave, not the bite). A jump peaks near 155, so a piece taller than that cannot be jumped onto or over. |
