...
describe('studyBanner', () => {
  it('shows the study text for the first 60 updates of the study only', () => {
    ...
    expect(studyBanner(active, 59)).toBe('Study: watch what it can do. Nothing can hurt you.');
    expect(studyBanner(active, 60)).toBeNull();
    expect(studyBanner(active, 500)).toBeNull();
  });

  it('says the fight begins for 60 updates after the study ends', () => {
    expect(studyBanner(ended(300), 300)).toBe('The fight begins!');
    expect(studyBanner(ended(300), 359)).toBe('The fight begins!');
    expect(studyBanner(ended(300), 360)).toBeNull();
  });
